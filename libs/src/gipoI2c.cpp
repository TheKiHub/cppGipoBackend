...
gipoI2c::gipoI2c(uint8_t busAddress) : m_i2c{mraa::I2c(0)} {
    LOG_DEBUG(m_logger, "I2C MRAA initialized using address: {}", busAddress);
...
gipoI2c::gipoI2c (uint8_t busAddress, uint8_t busNumber) : m_i2c{mraa::I2c(busNumber)} {
    LOG_DEBUG(m_logger, "I2C MRAA initialized on bus: {} using address: {}", busNumber, busAddress);
    m_i2cAddress = busAddress;
    m_i2c.address(busAddress);
}
...
uint8_t gipoI2c::I2CWriteReg8 (uint8_t reg, uint8_t data) {
    ...
    gipoI2c::gipoI2c(uint8_t busAddress) {
        m_i2cAddress = busAddress;
        LOG_DEBUG(m_logger, "I2C ready on address: {}", busAddress);
    }

    gipoI2c::gipoI2c(uint8_t busAddress, uint8_t busNumber) {
        m_i2cAddress = busAddress;
        LOG_DEBUG(m_logger, "I2CSetup ready on bus: {} with address: {}", busNumber, busAddress);
    }

    uint8_t gipoI2c::I2CWrite(uint8_t data) {
        LOG_DEBUG(m_logger, "I2CWrite address: {} data: {}", m_i2cAddress, data);
        return 0;
    }

    uint8_t gipoI2c::I2CWriteReg8(uint8_t reg, uint8_t data) {
        LOG_DEBUG(m_logger, "I2CWriteReg8 address: {} register: {} data: {}", m_i2cAddress, reg, data);
        return 0;
    }

    uint8_t gipoI2c::I2CRead() {
        LOG_DEBUG(m_logger, "I2C read byte on address: {}", m_i2cAddress);
        return 0;
    }

    uint8_t gipoI2c::I2CReadReg8(uint8_t reg) {
        LOG_DEBUG(m_logger, "I2C read register: {} on address: {}", reg, m_i2cAddress);
        return 0;
    }
#endif